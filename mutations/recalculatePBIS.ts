...
async function recalculatePbis(root: any,
  ...
  const student = await context.lists.User.findOne({
    ...
    resolveFields: graphql`
            id
             cards: _studentPbisCardsMeta(where:{counted:false}){
             count
            }
             yearCards: _studentPbisCardsMeta{
             count
            }
            taTeacher{
        id
  ...
  const taTeacher = await context.lists.User.findOne({
    ...
    resolveFields: graphql`
        id
         cards: _teacherPbisCardsMeta(where:{counted:false}){
         count
        }
         yearCards: _teacherPbisCardsMeta{
         count
        }
        taTeam{
            id
